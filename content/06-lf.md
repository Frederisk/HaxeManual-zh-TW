...
- Haxe Code Cookbook 中的[有關靜態延伸的 Haxe 程式碼片段與教學](http://code.haxe.org/tag/static-extension.html)。
...
Haxe 標準函式庫中的下列類是為了作為靜態延伸而設計的：

- [StringTools](https://api.haxe.org/StringTools.html)：提供字串的擴充功能，比如替換或者修剪。
- [Lambda](https://api.haxe.org/Lambda.html)：提供可疊代物件的功能性方法。
- [haxe.EnumTools](https://api.haxe.org/haxe/EnumTools.html)：提供有關枚舉及其執行個體型式資訊功能。
- `haxe.macro.Tools`：為巨集的相關作業提供不同的延伸。

> #### 瑣事：使用 using
>
> 自從語言中加入了 `using` 關鍵字之後，就會很常談到「使用 using」的某些問題，或者說「使用 using」的效果。這在很多情況下會讓英語的表達便笨拙，所以本手冊的作者決定按照實際的功能來稱呼該特徵：靜態延伸。
...
預設情況下，將類別用作靜態延伸會將其所有靜態方法引入上下文，從而容許它們成為適當型式的延伸。在一些情況中，類別可以提供其他不用做靜態延伸的靜態方法。可以用 `@:noUsing` 標記這些方法以確保他們不會干擾型式的正確方法。
...
透過使用 `@:using(args...)` 元資料標記型式可以為給定的型式始終啟用特定的靜態延伸。其中的引數應填入型式所將套用的靜態延伸的完整路徑：
...
class Main {
  static public function main() {
    ...
    // 會工作，即便在這個模組中並沒有 using Main.TreeTools
    trace(a.sum());
  }
}
```
...
模式匹配是根據給定的匹配值（可能是深模式）來分支處理的過程。在 Haxe 中，模式匹配主要在 [`switch` 運算式]{expression-switch}中完成，其中各個 `case` 運算式表述了模式。在此處，我們將以該資料結構為執行案例來探索不同模式的語法：
...
- 模式總是會自頂向底匹配。
- 匹配輸入值的最頂部模式會執行其運算式。
- `_` 會模式匹配所有東西，因此 `case _:` 等於 `default:`。

#### 相關內容

- 有關 [switch 運算式](expression-switch)的更多。
- Haxe Code Cookbook 中的[有關模式匹配的 Haxe 程式碼片段與教學](http://code.haxe.org/tag/pattern-matching.html)。
...
模式匹配器將自頂向底檢查每種狀況並選擇首個與輸入值匹配的。下列對每種情況的規則的手動解釋會有助於理解該過程：

- `case Leaf(_)`：匹配失敗，因為 `myTree` 是 `Node`
- `case Node(_, Leaf(_))`：匹配失敗，因為 `myTree` 的右子樹不是 `Leaf` 而是 `Node`<!--TODO: but another `Node`-->
- `case Node(_, Node(Leaf("bar"), _))`：匹配成功
- `case _`：在此處不會檢查，因為在前一列已匹配

<!--label:lf-pattern-matching-variable-capture-->
### 變數擷取

可以透過匹配子模式與識別符來擷取前者的任何值：
...
- 若 `myTree` 是 `Leaf`，將回傳其名稱。
- 若 `myTree` 是 `Node`，且其左子樹是 `Leaf`，將回傳其名稱（此例滿足這個條件，將回傳 `"foo"`）。
- 否則回傳 `"none"`。
...
在此處，若輸入匹配則 `leafNode` 會繫結至 `Leaf("foo")`。而在其他所有情況下，則會回傳 `meTree` 本身： `case x` 與 `case _` 的工作原理類似，兩者都會匹配任意內容。不過用類似 `x` 這樣的識別符名稱會將其匹配值繫結至該變數。

#### 自 Haxe 4.0.0

若偵測到「捕捉所有」變數的名稱與枚舉建構式十分相似，那編譯器會警告可能存在有拼寫錯誤。這個問題可以透過在變數識別符前加上 `var` 來規避：
...
switch (myTree) {
  ...
  // ...
  case var node:
    // 上面內容沒有 var 的話編譯器會警告可能有潛在的拼寫錯誤
    // node 將包含有整個匹配的樹
}
```

所有的擷取變數都可在前面加上 `var`，包括子匹配。
...
在第二種情況中，若 `rating` 匹配到 `"awesome"`，我們就會將匹配的 `name` 欄位繫結至識別符 `n`。當然這個例子也可以結合結構與枚舉匹配放到前面的 `Tree` 例子中。
...
The type(型式/型式化|n./v. 又：型別) `MyStringIterator` in this example qualifies as iterator(疊代器|): It define(定義|)s a method(方法|) `hasNext` return(回傳|)ing `Bool` and(及|gate) a method(方法|) `next` return(回傳|)ing `String`, making it compatible with `Iterator<String>`. The `main` method(方法|) instantiate(具現化|)s it, then iterate(疊代|)s over it.
