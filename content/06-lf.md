<!--label:lf-->
## Language feature(特徵|)s

**[abstract(抽象|) type(型式/型式化|n./v. 又：型別)s](types-abstract):**

An abstract(抽象|) type(型式/型式化|n./v. 又：型別) is a compile-time(編譯期|又：編譯時) construct(建構/構造|v./n.) which is represent(表述|)ed in a different way at runtime. This allow(容許|又：允許)s giving a whole new meaning to existing type(型式/型式化|n./v. 又：型別)s.

**[extern(外部|) class(類別|)es](lf-externs):**

extern(外部|)s can be used to describe(描述|) target(目標|)-specific(特定|) interaction in a type-safe(型式安全|) manner.

**[anonymous(匿名|) structure(結構|)s](types-anonymous-structure):**
