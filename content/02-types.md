...
Haxe 編譯器採用豐富的型式系統，其有助於在編譯時檢測程式中與型式相關的錯誤。型式錯誤是指對給定型式以無效操作的情況，例如嘗試除以一個字串、嘗試對數字的欄位存取或在呼叫一個函式時使用過多或過少的引數。

在某些語言中，這種額外的安全性是有代價的，程式設計師不得不將型式明確指派給語法結構：
...
我們將在稍後的[型式推理](type-system-type-inference)中詳細探討 Haxe 的型式推理。目前，只需要說明上述程式碼中的變數 `myButton` 是一個 `MySpecialButton` **類的執行個體**即可。
...
雖然每個 `Int` 都可以在預期為 `Float` 的地方使用，也就是說，`Int` 可**指派為** `Float` 或者是與 `Float` **相統一**，但是事實並非如此：將 `Float` 指派為 `Int` 可能會導致精度損失，因此並不允許這樣的隱含轉換。
...
> #### 定義：可空
>
> 在 Haxe 中，如果 `null` 可以指派給一個型式，那這個型式就是可空的。
...
因此，Haxe編譯器並不容許將 `null` 指派至靜態目標上的基本型式。為了實現指派 `null` ，基本型式必須首先包裝為 `Null<T>`：
...
如果 `Null<T>` 或 `Dynamic` 「隱含」有 `null` 值並指派給了基本型式，則受指派者會使用預設值：
...
> #### 定義：`Class<T>`
>
> 該型式與所有類別型式相容，這意味著所有類別都可以指派給它。不過類別的執行個體不能指派給該類別。
>
> 在編譯時，`Class<T>` 是所有類別型式的共同基底型式。這種關係不會反映在產生程式碼中。
>
...
類別執行個體是通過呼叫類別建構式建立的，這個過程通常稱作**具現化**。類別執行個體的另一個名稱是**物件**。儘管如此，我們更喜歡用術語類別執行個體來強調類別執行個體與[枚舉執行個體](types-enum-instance)的相似。

```haxe
var p = new Point(-1, 65);
```

上述程式碼將產生類別 `Point` 的執行個體，該執行個體指派給了名為 `p` 的變數。`Point`的建構式接收到了兩個引數 `x` 和 `y`（可在[類別執行個體](types-class-instance)比較其定義）。我們將在之後的 [new](expression-new) 部分重新審視 `new` 運算式的確切含意。目前，先將其視為呼叫類別建構式並返回了適合的物件。
...
這種關係通常描述做「是一個」：任何 `Point3` 類別的執行個體也都是 `Point` 的執行個體。`Point` 是 `Point3` 的**父類別**，`Point3` 是 `Point` 的**子類別**。一個類別可以有多個子類別，但是只有一個父類別。術語「類別 X 的父類別」通常是說它的直接父類別、他的父類別的父類別，以此類推。
...
介面可視作類別的簽章，它描述了類別公用欄位。介面並不提供實作，而只是提供純粹的結構資料。
...
此處的 `implements` 關鍵字表明 `Point` 與 `Printable` 之間存在著「是一個」的關係，比如說每一個 `Point` 的執行個體都是 `Printable` 的執行個體。雖然一個類別只能有一個父類別，但可以透過多個 `implements` 關鍵字實作多個介面：
...
抽象類別（不要與[抽象](types-abstract)相混淆）是具有部分實作的類別。因此，抽象類別無法實作而必須首先擴充，其子類要麼提供所有抽象方法的實作，要麼宣告自己也是抽象的。
...
類別即便沒有抽象方法也可宣告為抽象的。在此例中，這個抽象類別無法具現化，但是非抽象的子類別可以。
...
儘管其無法具現化，但抽象類別依然可以有建構式，其可以由子類別透過 `super()` 呼叫。
...
> #### 定義：`Enum<T>`
>
> 此型式與所有枚舉型式相容。在編譯期 `Enum<T>` 可視作所有枚舉型式的共同基底型式，不過這種關係不會反映在產生程式碼中。

<!--label:types-enum-constructor-->
### 枚舉建構式

與類別與其建構式類似，枚舉可透過建構式具現化。不過與類別同的是，枚舉提供了多個建構式，這些建構式可以透過各自的名稱呼叫：
...
在此處的程式碼中，`a`、`b` 和 `c` 是 `Color` 枚舉的執行個體，變數 `c` 以引數的方式呼叫 `Rgb` 建構式具現化。

所有的枚舉執行個體都可指派至一個特殊型式 `EnumValue`。

> #### 定義：EnumValue
>
> EnumValue 是一種與所有枚舉執行個體相統一的特殊型式。其由[標準函式庫](std)所用於為所有枚舉執行個體提供某些操作，以及在 API 需要的情況下容許在用戶程式碼中使用不特定枚舉執行個體。
...
如果對註釋內容取消註釋，程式將因為 `Red`（枚舉建構式）無法指派給型式為 `Enum<Color>`（枚舉型式）而無法編譯。這種關係和類別與其實作的關係是類似的。
...
結構的欄位也可宣告為 `final`，這將僅容許其指派一次。這樣的結構將只會與其他對應欄位同為 `final` 的型式相[統一](type-system-unification)。
...
函式 `test` 有兩個任選引數：`Int`型式的 `i` 以及 `String` 型式的 `s`。這將直接反映在第四列的函式型式輸出中。這個樣例程式呼叫了 `test` 四次，並列印出了各自的回傳值。
...
原理是如果所指派可由之後的引數接受，那麼前面的引數是可跳過的。
...
該樣例與[任選引數](types-function-optional-arguments)十分相似，唯一的不同是引數 `i` 與 `s` 分別指派為了 `12` 和 `"bar"`。此時在呼叫時省略引數，則會使用預設值而不是 `null`。

Haxe 中的預設值不是型式的一部分並且除非函式是[內聯](class-field-inline)的，否則不會在呼叫處替換。在某些平台上，編譯器可能在省略引數時仍然傳入 `null`，而在函式內產生的程式碼看上去會是類似這樣：
...
雖然 Haxe 有靜態型式系統，但其本質上可以透過使用 `Dynamic` 型式來關閉。**動態值**可以以任何變數指派，也可以指派給任何變數。這有幾個缺點：

- 編譯器將不再對預期特定型式的指派、函式呼叫以及其他建構子執行型式檢查。
...
如前所述，抽象是編譯期功能，所以觀察上面所實際產生的東西會很有趣，由於 JavaScript 傾向產生簡潔清晰的程式碼所以比較合適作為目標。以指令 `haxe --main MyAbstract --js myabstract.js` 編譯會得到以下 JavaScript 程式：
...
我們宣告了 `MyAbstract` 是 `from int` 和 `to Int` 的，也就意味著其可以從 `Int` 指派也可以指派給 `Int`。第 9 到 10 列展示了這一點，在此處我們首先將 `Int` 的 `12` 指派給了 `MyAbstract` 的變數 `a` ，由於 `from Int` 宣告，這可以正常工作；之後那個抽象又返回給了 `Int` 型式的變數 `b` ，由於 `to Int` 宣告，這也可以正常工作。
...
在前面的例子中，方法 `fromString` 容許將值 `"3"` 指派至型式為 `MyAbstract` 的變數 `a`，而方法 `toArray` 容許將該抽象指派至 `Array<Int>` 型式變數 `b` 的內部。
...
`@:op` 函式的方法本體可以省略，但前提是抽象的底層型式容許相關運算，並且結果的型式可以指派回抽象。
...
該例子中的 `MyArray` 抽象包裝了 `Array`，其 `@:forward` 元資料有兩個引數分別對應需要轉送至底層型式的欄位名稱。在此例中，`main` 方法具現化了 `MyArray` 並存取了它的 `push` 和 `pop` 方法。註釋表明 `length` 欄位不可用。

和往常一樣，我們可以透過 JavaScript 輸出以了解程式碼是如何產生的：
